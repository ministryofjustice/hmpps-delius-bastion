...
data "terraform_remote_state" "vpc" {
  ...
  config {
    ...
    key    = "vpc-bastion/terraform.tfstate"
    region = "${var.region}"
  }
}
...
module "auto_scale" {
  ...
  subnet_ids = ["${element(data.terraform_remote_state.vpc.private-subnet-az1,0)}",
    "${element(data.terraform_remote_state.vpc.private-subnet-az2,0)}",
    "${element(data.terraform_remote_state.vpc.private-subnet-az3,0)}",
  ]
